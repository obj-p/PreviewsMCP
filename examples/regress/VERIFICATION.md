...
| B04 | Reproduced | The dynamic-only package passed natively and PreviewsMCP loaded it on iOS: the snapshot rendered `Dynamic simulator XCFramework`. The same snapshot reported `framework resource missing`, so the framework's internal JSON was not staged with the loaded binary. Re-verified 2026-07-21: identical behavior. |
...
| R02 | Reproduced | English JSON, text, and localization resources rendered on macOS and iOS. Both selecting preview index 1 and an explicit single-preview Spanish localization control produced a blank or partial framebuffer on iOS. Native SwiftPM build passed and both locale directories were present in the staged bundle. Re-verified 2026-07-21: the blank/partial framebuffer no longer reproduces — index 1 after a switch and the Spanish-only control both render on iOS. Every surface (iOS, macOS, `--locale es`, and a native harness against the built bundle with both `.lproj` directories present) rendered the English title: `String(localized:bundle:locale:)` does not select the `.lproj` (its `locale:` parameter affects interpolation formatting only), so the fixture cannot display Spanish even when staging is correct. The localization half needs a fixture correction (SwiftUI `Text` with the `\.locale` environment, or an explicit lproj sub-bundle lookup) before the product gap can be judged. JSON and text resources stage and render. |
