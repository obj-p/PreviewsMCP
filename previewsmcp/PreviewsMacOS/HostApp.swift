...
public class PreviewHost: NSObject, NSApplicationDelegate {
    ...
    public func watchFile(
        ...
        let watchSet = Self.watchSet(primary: filePath, buildContext: buildContext)
    ...
    /// The exact paths and directory roots a session watches. Paths that no
    /// longer exist are dropped (FileWatcher refuses missing paths, and a
    /// refresh triggered by a deletion must still reinstall its watcher);
    /// they rejoin the set when the next refresh re-captures them.
    static func watchSet(
