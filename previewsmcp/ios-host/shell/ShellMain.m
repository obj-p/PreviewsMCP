...
- (id)clientIdentityForToken:(pvm_audit_token_t)tok {
  ...
  return [ClientId performSelector:@selector(identityForProcessIdentity:)
                        withObject:procIdentity];
}
...
- (void)hostWithToken:(pvm_audit_token_t)tok {
  ...
  void (^settingsUpdater)(id) = ^(id settings) {
    ...
    if ([settings respondsToSelector:@selector(setForeground:)])
      ((void (*)(id, SEL, BOOL))objc_msgSend)(settings,
                                              @selector(setForeground:), YES);
    if ([settings respondsToSelector:@selector(setBackgrounded:)])
      ((void (*)(id, SEL, BOOL))objc_msgSend)(settings,
                                              @selector(setBackgrounded:), NO);
  };
  if ([adv respondsToSelector:@selector(setInitialSettingsUpdater:)])
    [adv performSelector:@selector(setInitialSettingsUpdater:)
              withObject:settingsUpdater];
  self.host =
      [[HostC alloc] performSelector:@selector(initWithAdvancedConfiguration:)
                          withObject:adv];
  ...
  @try {
    ...
    if (self.fgAssertion) {
      if ([self.fgAssertion respondsToSelector:@selector(invalidate)])
        ((void (*)(id, SEL))objc_msgSend)(self.fgAssertion,
                                          @selector(invalidate));
      self.fgAssertion = nil;
    }
    self.fgAssertion =
        [comp performSelector:@selector(foregroundAssertionForReason:)
...
- (void)startSnapshotCache {
  ...
  dispatch_source_set_timer(
      t, dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)),
      (uint64_t)(1.0 * NSEC_PER_SEC), (uint64_t)(0.3 * NSEC_PER_SEC));
  __weak typeof(self) weakSelf = self;
  dispatch_source_set_event_handler(t, ^{
    typeof(self) self2 = weakSelf;
    if (!self2 || !self2.window || self2.overlay)
      return;
    UIView *v = self2.window;
    UIGraphicsImageRenderer *r =
        [[UIGraphicsImageRenderer alloc] initWithBounds:v.bounds];
    self2.cachedFrame =
        [r imageWithActions:^(UIGraphicsImageRendererContext *_Nonnull ctx) {
          (void)ctx;
...
// becomes readable and recv() returns 0. That is the flash-free respawn
// trigger.
- (void)watchAgentDeath:(int)fd {
  dispatch_source_t s = dispatch_source_create(DISPATCH_SOURCE_TYPE_READ, fd, 0,
                                               dispatch_get_main_queue());
  __weak typeof(self) weakSelf = self;
  dispatch_source_set_event_handler(s, ^{
    ...
    if (n > 0)
      return; // unexpected data from the agent; ignore
    typeof(self) self2 = weakSelf;
    if (!self2 || self2.agentFD != fd)
      return;
    [self2 onAgentDeath];
  });
  dispatch_resume(s);
...
// Hold the cached frame under a dim scrim immediately (flash-free), then
// escalate: a spinner appears only if the respawn is slow, and a disconnected
// icon replaces it if the agent never comes back. A successful re-host calls
// hideOverlay, which cancels both pending transitions.
- (void)showOverlay {
  if (self.overlay)
    return;
...
- (UIActivityIndicatorView *)centeredSpinner {
  ...
  spin.autoresizingMask = UIViewAutoresizingFlexibleTopMargin |
...
- (void)showDisconnected {
  if (!self.overlay || self.disconnectedIcon)
    return;
  if (self.spinner) {
  ...
  iv.center = CGPointMake(self.overlay.bounds.size.width / 2.0,
                          self.overlay.bounds.size.height / 2.0);
  iv.autoresizingMask = UIViewAutoresizingFlexibleTopMargin |
                        UIViewAutoresizingFlexibleBottomMargin |
                        UIViewAutoresizingFlexibleLeftMargin |
                        UIViewAutoresizingFlexibleRightMargin;
...
- (UISceneConfiguration *)application:(UIApplication *)app
  ...
  UISceneConfiguration *c =
      [UISceneConfiguration configurationWithName:@"Default"
                                      sessionRole:s.role];
