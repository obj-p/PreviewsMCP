...
6. **Verify setup.** Run `bazel --version`, `swift-format --version`, `swiftlint version`, and `swift --version` to confirm tool availability. If `--examples` was passed, also verify `mint version`.
