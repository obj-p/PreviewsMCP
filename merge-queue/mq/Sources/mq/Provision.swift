...
enum Provision {
    static func toolchain(
        ...
        if try await guest.test("test -d \(xcodeApp)") {
            ...
            try await guest.upload(localPath: xcodeXip, to: "/tmp/Xcode.xip")
        ...
        try await guest.sh(
            ...
            timeout: 1800)
        try await guest.sh(
            "eval \"$(/opt/homebrew/bin/brew shellenv)\" && brew install bazelisk mise",
            timeout: 1800)

        print("==> enabling autologin for \(adminUser)")
        let encoded = kcpassword(guest.adminPass).base64EncodedString()
        try await guest.sudo(
            "bash -c 'echo \(encoded) | base64 -D > /etc/kcpassword "
    ...
    static func kcpassword(_ password: String) -> Data {
