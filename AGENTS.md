...
- **Daemon-global heartbeat** — the MCP server emits an unconditional `LogMessageNotification` with `logger: "heartbeat"` every 2s for the lifetime of a connected transport. This is the liveness signal Phase 2's client-side stall detector will key off of; it fires whether or not any tool call is in flight, which is necessary because the FileWatcher hot-reload path is entirely outside any request scope. `DaemonClient.registerStderrLogForwarder` filters these notifications out so they don't spam the CLI's stderr. See `runMCPServer` in `MCPServer.swift`. Do NOT replace `server.log(level:,logger:,data:)` with `ProgressNotification` — unsolicited progress notifications without a `progressToken` are out-of-spec and will confuse MCP agents.
