...
- **MCPTestServer watchdog** — every active `MCPTestServer` schedules a `DispatchSourceTimer` that fires every 60s on a libdispatch queue and writes the elapsed time plus the tail of the server's stderr to the test process's own stderr via `fputs`. Silent on the happy path (tests < 60s never emit), but when Swift Testing's `.timeLimit` kills a hanging MCP test the CI log now contains a minute-by-minute trail of what the server subprocess was actually doing. Don't replace it with a `Task.sleep`-based watchdog: the whole point is to survive cooperative-pool starvation.
